...
Graph* GraphCreateTranspose(const Graph* g) {
  ...
  Graph* New_Graph = GraphCreate(g->numVertices, g->isDigraph,g->isWeighted); // create a new graph
  ListMoveToHead(g->verticesList);                                            // move to first node of the vertex list

  for (unsigned int i = 0; i < g->numVertices; ListMoveToNext(g->verticesList), i++){  // for each vertex on this list...
    
    struct _Vertex* v = ListGetCurrentItem(g->verticesList);                  // get the current vertex

    List* edges = v->edgesList;                                               //// get the list of edges correspondant to the
                                                                              ////  vertex
    unsigned int edgessize = ListGetSize(edges);                              // get the size of that list
    ListMoveToHead(edges);                                                    // move to first node of the edges list

    //for weighted graph
    if (g->isWeighted){
      for (unsigned int j = 0; j < edgessize; ListMoveToNext(edges), j++){               // for each edge on this list...

        struct _Edge* edge = ListGetCurrentItem(edges);                         // get the current edge
        GraphAddWeightedEdge(New_Graph, edge->adjVertex, v->id, edge->weight);  //// add the new edge directed edge (starting 
                                                                                ////  in the adjvertex and ending in the initial                                                                      ////  vertex, with the corresponding weight)
      }
    //for non weighted graph
        ...
        struct _Edge* edge = ListGetCurrentItem(edges);                         // get the current edge
        GraphAddEdge(New_Graph, edge->adjVertex, v->id);                        //// add the new edge directed edge (starting 
                                                                                ////  in the adjvertex and ending in the initial                                                                      ////  vertex, with the corresponding weight)
      }
    }
    
  }
  return New_Graph;                                                           // return the new Transpose graph
}
